...
В \t{examplethree} "--- три. В случае применения \command{exmpfile} первые два
аргумента рассматриваются как файлы в текущей директории, откуда
необходимо считать тест.  Для использования \command{exmpfile} необходимо подключить пакет \t{verbatim}.
...
команду \command{disablemathrussificationtrue} в преамбуле документа.
